...
namespace Zadacha06
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                case "fragile":
                    var fragile = cars.Where(x => x.CarTyres[0].Naliagane < 1 &&
                    x.CarTyres[1].Naliagane < 1 && x.CarTyres[2].Naliagane < 1 &&
                    x.CarTyres[3].Naliagane < 1).Select(y => y.CarModel.CarModel).ToList();
